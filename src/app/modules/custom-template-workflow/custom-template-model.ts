...
export class CustomField {
  ...
  constructor(obj: any) {
    ...
    this.workflowTemplateId = obj['workflow_template_id'] ?? obj['workflowTemplateId'] ?? '';
...
export class TaskOutputModel {
  ...
  constructor(obj) {
    ...
    this.assigned_to = obj.user?.length ? obj.user[0]?.id : '';
...
export class Task {
  ...
  constructor(obj: any) {
    ...
    this.taskName = obj.taskName ?? (obj['task_name'] || obj['name']);
    this.importance = importance[obj.importance] ?? obj.importance;
    this.description = obj.description ?? '';
    this.workFlow = obj.workFlow ?? obj['workflow_id'] ?? [];
    this.user = obj.user ?? (obj['assigned_to_id'] || obj['assigned_to']) ?? [];
...
export class Workflow {
  ...
  fromOutOfPage: Boolean;
  constructor(obj: any) {
    this.name = obj.name ?? obj['workflow_name'];
    this.importance = importance[obj.importance] ?? obj.importance;
    this.description = obj.description ?? '';
    this.project = (Array.isArray(obj.project) ? obj.project : obj.project?.id) ?? obj['project_id'] ?? [];
...
export class CustomTemplate {
  ...
  constructor(obj: any) {
    ...
    this.tasks = obj.task_fixtures;
    this.isActive = obj['is_active'];
    this.id = obj['pk'];
    this.title = obj['title'];
    this.workflow = new Workflow(obj);
  }
}
...
export class CustomWorkflowTemplateOutputModel {
  ...
  constructor(obj) {
    ...
    this.project_id = Array.isArray(obj.project) ? (obj.project.length > 0 ? obj.project : null) : (obj.project ?? null)
    this.due_date = obj.dueDate;
    this.start_date = obj.startDate;
    this.is_private = obj.isPrivate ?? '';
    this.description = obj.description ?? '';
