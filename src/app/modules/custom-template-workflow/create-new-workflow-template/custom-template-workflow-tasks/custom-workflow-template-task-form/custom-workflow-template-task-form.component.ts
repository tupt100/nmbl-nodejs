...
@Component({
  selector: 'app-custom-workflow-template-task-form',
  templateUrl: './custom-workflow-template-task-form.component.html',
  styleUrls: ['./custom-workflow-template-task-form.component.scss']
})
export class CustomWorkflowTemplateTaskFormComponent implements OnInit, OnDestroy {
  @ViewChild(TaskPriorComponent) taskPrior: TaskPriorComponent;
  @ViewChild(NotifierComponent) notifier: NotifierComponent;

  @Input() initialData;
  @Input() index;
  @Output() onTaskChanged = new EventEmitter<any>();
  ...
  initView() {
    this.importance = 1;
    const privilegeList = this.sharedService.privilegeList.map(p => {
      return {...p}
    })
    this.arrPrivilege = privilegeList.filter((val: any) => {
      return val;
    });
    this.tagTitle = [];
    this.groupTitle = [];
  ...
  updateTask(): void {
    this.onTaskChanged.emit({
      ...
      privileges: this.arrPrivilege.filter(p => p.title !== 'None')
    });
  }
