 ...
 */
  ...
  searchTasks() {
    ...
    this.defaultParam.search = this.searchText;
    const ajax = this.customTemplateWorkflowService.getCustomWorkflowTemplates(params).toPromise();
    ajax.then(res => {
      if (res && res.results) {
  ...
  getCustomPage(url: string) {
    this.loading = false;
    this.customTemplateWorkflowService.getCustomTaskTemplatesPage(url).subscribe(res => {
      if (res) {
        this.loading = true;
        this.templates = res['results'];
        this.next = res.next;
  ...
  getWorkflowTemplates() {
    ...
    this.customTemplateWorkflowService.getCustomWorkflowTemplates(filteredParams).subscribe(res => {
      if (res) {
        this.loading = true;
        this.templates = res['results'];
        this.next = res.next;
  ...
  archiveTemplate($event) {
    if ($event && this.tasktemplateIdToDelete) {
      this.customTemplateWorkflowService.removeWorkflowTemplate(this.tasktemplateIdToDelete).pipe(takeUntil(this.ngDestroy$))
        .subscribe(data => {
