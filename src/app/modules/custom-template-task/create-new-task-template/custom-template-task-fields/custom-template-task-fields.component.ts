...
import { importanceLevel } from '../../custom-template-model';

@Component({
  selector: 'app-custom-template-task-fields',
  templateUrl: './custom-template-task-fields.component.html',
  styleUrls: ['./custom-template-task-fields.component.scss']
})
export class CustomTemplateTaskFieldsComponent implements OnInit, OnDestroy {
  ...
  taskFields$ = this.customTemplateFacade.getTaskField$;
  ...
  initView() {
    ...
    const inEditModeAndIsFirstLoad = this.templateId && CustomTemplateTaskService.prevStep == "new";
    const withBackButton = CustomTemplateTaskService.prevStep !== "new";
    if (inEditModeAndIsFirstLoad) {
      ...
      this.customTemplate$.pipe(takeUntil(this.ngDestroy$)).subscribe(data => {
        ...
        if (data['task'] && data.id == this.templateId) {
          this.updateValues(data['task']);
        }
      });
    } else if (withBackButton) {
      this.taskFields$.pipe(take(1)).subscribe(data => {
        if (Object.keys(data).length > 0 && this.taskForm) {

          data.workFlow = data.workFlow.length ? data.workFlow[0].id : data.workFlow;
  ...
  createTask() {
    ...
    const workFlow = this.workflowTitle;
    const user = this.userTitle;
    const isPrivate = this.isPrivate;
    const privileges = this.arrPrivilege.filter(p => p.title !== 'None');
    const startDate = this.startDate;
    const dueDate = this.dueDate;
    data = { ...data, workFlow, user, group, startDate, dueDate, isPrivate, privileges };
    this.customTemplateFacade.createTaskField(data);

    this.customTemplateTaskService.changeWizardStep('custom-field', 'task-fields');
  }
