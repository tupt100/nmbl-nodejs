...
export class PreviewTemplateWorkflowComponent implements OnInit, OnDestroy {
  ...
  findTasksDifference(taskTemplate: CustomTemplate) {
    return _.differenceWith(this.tasks, taskTemplate.workflows, _.isEqual);
  }
    ...
    try {
      ...
      if (this.templateId) {
        if (this.templateLabel.title !== this.form.get('title').value ||
          JSON.stringify(this.workflow) !== JSON.stringify(this.customTemplate.project)) {
            ...
            mergeMap(res => {
              ...
              if (differTasks.length > 0) {
                taskRequests = differTasks.map(task => {
                  if (!task.id) {
                    return this.customTemplateWorkflowService.createTask(new TaskOutputModel({ ...task, project_template_id: this.templateId, attachments: [] }))
                  } else if (task.deleted) {
                    return this.customTemplateWorkflowService.removeTask(task.id);
                  } else {
                    return this.customTemplateWorkflowService.updateTask(new TaskOutputModel({ ...task, project_template_id: this.templateId, attachments: [] }), task.id)
                  }
                })
              }
            ...
            mergeMap(res => {
              ...
              if (this.tasks?.length > 0) {
                taskRequests = this.tasks.filter(task => task && !task.deleted).map(task => {
                  return this.customTemplateWorkflowService.createTask(new TaskOutputModel({ ...task, project_template_id: res['pk'], attachments: [] }))
                })
              }
              const requests = [...customFieldRequests, ...taskRequests]
              if (requests.length > 0) {
  ...
  goToDashboard() {
    this.router.navigateByUrl('main/custom-template-project');
  }
