...
export class CustomTemplateWorkflowFieldsComponent implements OnInit, OnDestroy {
  ...
  initView() {
    ...
    if (inEditModeAndIsFirstLoad) {
      ...
      this.customTemplate$.pipe(takeUntil(this.ngDestroy$)).subscribe(data => {
        if (data) {
          const updateData = () => {
            ...
            if (data['project'] && data.id == this.templateId) {
              this.updateValues(data['project']);
            }
          }
  
          if (data.workflows?.length) {
            const taskRequests = data.workflows.map(task => {
      ...
      this.taskFields$.pipe(take(1)).subscribe(data => {
        if (Object.keys(data).length > 0 && this.projectForm) {
  ...
  createProject() {
    ...
    data = { ...data, user, group, startDate, dueDate, isPrivate, privileges };
