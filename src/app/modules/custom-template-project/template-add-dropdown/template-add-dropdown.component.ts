...
@Component({
  selector: 'app-project-template-add-dropdown',
  templateUrl: './template-add-dropdown.component.html',
  styleUrls: ['./template-add-dropdown.component.scss']
})
export class ProjectTemplateAddDropdownComponent implements OnInit, OnDestroy {
  templates = [];
  loading: boolean;
  searchText: String;
  timer: any;
  ...
  private objFeatures$;
  @Output() addNewProjectTemplate: EventEmitter<void> = new EventEmitter();
  constructor(
  ...
  goToAddTaskTemplate = () => {
    this.addNewProjectTemplate.emit();
  }
