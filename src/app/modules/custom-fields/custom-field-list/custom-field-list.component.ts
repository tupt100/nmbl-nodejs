 ...
 */
  ...
  searchTasks() {
    ...
    this.defaultParam.search = this.searchText;
    const ajax = this.globalCustomFieldService.getGlabalCustomFields(params).toPromise();
    ajax.then(res => {
      if (res && res.results) {
  ...
  getCustomPage(url: string) {
    this.loading = false;
    this.globalCustomFieldService.getCustomTaskTemplatesPage(url).subscribe(res => {
      if (res) {
        this.loading = true;
        this.templates = res['results'];
        this.next = res.next;
  ...
  getTaskTemplates() {
    ...
    this.globalCustomFieldService.getGlabalCustomFields(filteredParams).subscribe(res => {
      if (res) {
        this.loading = true;
        this.templates = res['results'];
        this.next = res.next;
  ...
  archiveTemplate($event) {
    if ($event && this.tasktemplateIdToDelete) {
      this.globalCustomFieldService.removeCustomField(this.tasktemplateIdToDelete).pipe(takeUntil(this.ngDestroy$))
        .subscribe(data => {
